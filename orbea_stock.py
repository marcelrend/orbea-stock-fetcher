...
class OrbeaStock:
    ...
    def login(self):
        ...
        if login_response.status_code != 303:
            raise Exception(f"Error: login returned status code [{login_response.status_code}], expected 303")
    ...
    def download(self):
        ...
        if download_response.status_code != 200:
            raise Exception(f"Error: csv download returned status code [{download_response.status_code}], expected 200")
